...
def compute_access_credentials_for_obu_on_td(device_contract_ref: str, rnd_obe:int, ac_cr_key_ref:int, td_name:str):
    # Compute the Access Key
    access_key = tc_td_key_derivation.compute_ack_for_obu_on_td(device_contract_ref, ac_cr_key_ref, td_name)
    # Compute the Access Credentials and return it
    return compute_access_credentials_with_access_key(rnd_obe, access_key)

def compute_access_credentials_with_efc_cm_only(efc_cm:str, rnd_obe:int, ac_cr_key_ref:int):
    # Compute the Access Key
    access_key = tc_td_key_derivation.compute_ack_with_efc_cm_only(efc_cm, ac_cr_key_ref)
    # Compute the Access Credentials and return it
    return compute_access_credentials_with_access_key(rnd_obe, access_key)
...
class InvalidAcKType(ValueError):
    pass

def compute_access_credentials_with_access_key(rnd_obe:int, access_key:bytes):
    if type(access_key) is not bytes:
        raise InvalidAcKType(f'Access Key should have type (bytes), not ({type(access_key)})!!')
...
def compute_authenticator_with_efc_cm_and_auk_ref(
        ...
        rnd_rse,
        auk_ref=115,
        norm:str='TIS_decimal',
    ) -> bytes:
    # Obtaining AuK via EFC-CM only info (not ideal!!)
    authenticator_key = tc_td_key_derivation.force_compute_auk_with_efc_cm_and_auk_ref_only(pan_bytes, efc_cm, auk_ref, norm)
...
def compute_authenticator_with_device_contract_ref_and_auk_ref(
        pan_bytes:bytes,
        device_contract_ref:str,
        attribute_list_bytes,
        rnd_rse,
        td_name:str,
        norm:str,
    ...
    authenticator_key = tc_td_key_derivation.compute_auk_with_device_contract_ref_auk_ref_on_td(pan_bytes, device_contract_ref, td_name, norm, auk_ref)

    attr_authenticator = compute_authenticator_with_auk_value(attribute_list_bytes, rnd_rse, auk_value=authenticator_key)
    dsrc_auth_logger.info(f"[OBE AUTH] Authenticator computed by RSE (UPER hex): {attr_authenticator.hex().upper()}")
    return attr_authenticator

def compute_authenticator_with_device_info_and_auk_ref(
        pan_bytes:bytes,
        efc_cm_hex_str: str,
    ...
    authenticator_key = tc_td_key_derivation.compute_auk_with_obu_info_and_auk_ref_on_td(pan_bytes, efc_cm_hex_str, manufacturer_id_hex_str, equipment_class_hex_str, norm, td_name, auk_ref)
