...
with open('../security_info/axxes_master_uset_key_hex_v1.0.txt', 'r') as txt_file:
    axxes_master_uset_key_hex = txt_file.read()
    axxes_master_uset_key_bytes = bytes.fromhex(axxes_master_uset_key_hex)

    axxes_uset_cipher_part1 = DES3.new(axxes_master_uset_key_bytes[0:5], DES3.MODE_ECB)
    axxes_uset_cipher_part2 = DES3.new(axxes_master_uset_key_bytes[5:10], DES3.MODE_ECB)
    del axxes_master_uset_key_bytes
    del axxes_master_uset_key_hex
...
def get_uset_derived_key_bytes(ac_cr_keyref:int) -> bytes:
...
def get_uset_derived_key_bytes_from_obu_id(eq_obu_id_bytes:bytes) -> bytes:
    ac_cr_keyref = get_kapsch_ac_cr_keyref_from_obu_id(eq_obu_id_bytes)
    return get_uset_derived_key_bytes(ac_cr_keyref)

def get_uset_derived_key_bytes_from_serial_number(serial_number:str) -> bytes:
